...
const ResetOrInitAsyncStorage = async (relatorioEnviado = false) => {
  let initialStateCopy = JSON.stringify(initialState);
  initialStateCopy = JSON.parse(initialStateCopy);
  initialStateCopy.relatorioEnviado = relatorioEnviado;
  await SaveAsyncStorage(initialStateCopy);
}

// Save os dados recebidos por parâmetros via AsyncStorage
const SaveAsyncStorage = async data => await AsyncStorage.setItem("dataSave", JSON.stringify(data));
...
export { ResetOrInitAsyncStorage, SaveAsyncStorage, GetAsyncStorage, initialState };
